...
if(isset($_POST['submit'])) {
    ...
    if($status == 2) {
        ...
        header("Location: ../admin_rejected_official_forms.php?form=deleted");
        exit();
    }
    else if($status == 3) {
...
Thank you';
    ...
    header("Location: ../admin_accepted_official_forms.php?form=accepted");
    exit(); 
    }
    else if($status == 0) {
        $sql = "UPDATE official_req SET status = '$status' WHERE form_no = '$form_no';";
        mysqli_query($conn, $sql);
        header("Location: ../admin_official_forms.php?form=pending");
        exit();
