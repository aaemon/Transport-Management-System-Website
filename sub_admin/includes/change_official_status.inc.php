...
if(isset($_POST['submit'])) {
    ...
    /*if($status == 2) {
        $sql = "DELETE FROM official_req WHERE form_no = '$form_no';";
        mysqli_query($conn, $sql);

    }*/
    if($status == 3) {
        $sql = "UPDATE official_req SET status = '$status' WHERE form_no = '$form_no';";
        mysqli_query($conn, $sql);
        $to = $user_email;
        $subject = 'Request Rejected';
        $message_body = 'Hello,' .$user_name .',
        
Sorry, your application (Official Requisition Form) has been rejected.
...
Thank you';
        ...
        header("Location: ../sub_admin_rejected_official_forms.php?form=deleted");
        exit();
    }
    else if($status == 1) {
        $sql = "UPDATE official_req SET status = '$status' WHERE form_no = '$form_no';";
...
Congratulations, your application (Official Requisition Form) has been accepted.
...
Thank you';
    ...
    header("Location: ../sub_admin_accepted_official_forms.php?form=accepted");
    exit(); 
    }
    else if($status == 0) {
        $sql = "UPDATE official_req SET status = '$status' WHERE form_no = '$form_no';";
        mysqli_query($conn, $sql);
        header("Location: ../sub_admin_official_forms.php?form=pending");
        exit();
    ...
    $pdf->Ln();
    $pdf->Cell(95, 8, "OFFICIAL REQUISITION FORM",'C');
    $pdf->Ln();
    ...
    $pdf->Ln();
    $pdf->Cell(45,12,"Token: ",1);
    $pdf->Cell(145,12,$token,1);
    $pdf->Ln();
